...
public class MessageFactory<T extends IsoMessage> {
	 ...
	 * 
	 * @param type
	 *            The message type, for example 0x200, 0x400, etc.
	 */
	...
	public void setTimezoneForParseGuide(int messageType, int field, TimeZone tz) {
		...
		log.warn(
				"Field {} for message type {} is not for dates, cannot set timezone",
				field, messageType);
	}
	 ...
	 *            The expected length of the ISO header, after which the message
	...
	public T parseMessage(byte[] buf, int isoHeaderLength)
		...
		if (index == null) {
			log.error(String
					.format("ISO8583 MessageFactory has no parsing guide for message type %04x [%s]",
							type, new String(buf)));
			return null;
		}
		...
		for (int i = 1; i < bs.length(); i++) {
			if (bs.get(i) && !index.contains(i + 1)) {
				log.warn(
						"ISO8583 MessageFactory cannot parse field {}: unspecified in parsing guide",
						i + 1);
				abandon = true;
			}
		}
			...
			for (Integer i : index) {
				...
				if (bs.get(i - 1)) {
					if (ignoreLast && pos >= buf.length
						...
						log.warn(
								"Field {} is not really in the message even though it's in the bitmap",
								i);
						bs.clear(i - 1);
					} else {
						...
						if (val != null) {
							...
							if (val.getType() == IsoType.LLVAR
									...
									|| val.getType() == IsoType.LLBINBCD
									|| val.getType() == IsoType.LLVARBCD) {
								pos++;
	 ...
	 * 
	 * @param value
	 *            A map where the keys are the message types and the values are
	 *            the ISO headers.
	 */
	 ...
	 * 
	 * @param type
	 *            The message type, for example 0x200.
	 * @param value
	...
	public void setParseMap(int type, Map<Integer, FieldParseInfo> map) {
		...
		log.trace(String
				.format("ISO8583 MessageFactory adding parse map for type %04x with fields %s",
						type, index));
