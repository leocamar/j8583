...
public final class Bcd {
	...
	public static void encode(String value, byte[] buf) {
		...
		int bufpos = 0;
		if (value.length() % 2 == 1) {
			if (value.charAt(0) > 57) {
				buf[0] = (byte) (value.charAt(0) - 87);
				charpos = 1;
				bufpos = 1;
		...
		while (charpos < value.length()) {
			buf[bufpos] = (byte) ((value.charAt(charpos) > 57 ? ((value
					.charAt(charpos) - 87) << 4)
					: ((value.charAt(charpos) - 48) << 4))
							| ((value.charAt(charpos + 1) > 57) ? (value
							.charAt(charpos + 1) - 87) : (value
							.charAt(charpos + 1) - 48)));
			charpos += 2;
			bufpos++;
		}
