 ...
 */
public class LllbinFEncodeParseInfo extends FieldParseInfo {

	
	public LllbinFEncodeParseInfo() {
		super(IsoType.LLLBINFENCODE, 0);
	}
	...
	public <T> IsoValue<?> parse(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format("Invalid LLLBINFENCODE field %d pos %d",
                    field, pos), pos);
		} else if (pos+3 > buf.length) {
			throw new ParseException(String.format("Insufficient LLLBINFENCODE header field %d",
                    field), pos);
		}
		final int l = decodeLength(buf, pos, 3);
		if (l < 0) {
			throw new ParseException(String.format("Invalid LLLBINFENCODE length %d field %d pos %d",
                    l, field, pos), pos);
		} else if (l+pos+3 > buf.length) {
			throw new ParseException(String.format(
                    "Insufficient data for LLLBINFENCODE field %d, pos %d", field, pos), pos);
		}
	...
	public <T> IsoValue<?> parseBinary(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format("Invalid bin LLLBINFENCODE field %d pos %d",
                    field, pos), pos);
		} else if (pos+2 > buf.length) {
            throw new ParseException(String.format("Insufficient LLLBINFENCODE header field %d",
                             field), pos);
		}
		final int l = ((buf[pos] & 0x0F) * 100) + (((buf[pos + 1] & 0x0F)) * 10) + (buf[pos + 2] & 0x0F);
		if (l < 0) {
            throw new ParseException(String.format("Invalid LLLBINFENCODE length %d field %d pos %d",
                             l, field, pos), pos);
		}
		if (l+pos+2 > buf.length) {
			throw new ParseException(String.format(
                    "Insufficient data for bin LLLBINFENCODE field %d, pos %d requires %d, only %d available",
