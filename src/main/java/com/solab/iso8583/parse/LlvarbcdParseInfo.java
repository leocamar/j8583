 ...
 */
public class LlvarbcdParseInfo extends FieldParseInfo {

	public LlvarbcdParseInfo() {
		super(IsoType.LLVARBCD, 0);
	}
	...
	public <T> IsoValue<?> parse(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid LLVARBCD field %d position %d", field, pos), pos);
		} else if (pos + 2 > buf.length) {
			throw new ParseException(String.format(
					"Insufficient LLVARBCD header field %d", field), pos);
		}
		length = decodeLength(buf, pos, 2);
		if (length < 0) {
			throw new ParseException(String.format(
					"Invalid LLVARBCD field %d length %d pos %d", field,
					length, pos), pos);
		}
		if (length + pos + 2 > buf.length) {
			throw new ParseException(
					String.format(
							"Insufficient data for LLVARBCD field %d, pos %d (LEN states '%s')",
	...
	public <T> IsoValue<?> parseBinary(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid bin LLVARBCD field %d position %d", field, pos),
					pos);
		} else if (pos + 1 > buf.length) {
			throw new ParseException(String.format(
					"Insufficient bin LLVARBCD header field %d", field), pos);
		}
		
		
		int l = Integer.parseInt(
				String.valueOf((((buf[pos] & 0xf0) >> 4) * 16)
						+ (buf[pos] & 0x0f)));
		if ((l % 2) != 0){
			l++;
			l = l/2;
		...
		if (l < 0) {
			throw new ParseException(String.format(
					"Invalid bin LLVARBCD length %d pos %d", length, pos), pos);
		}
		if (l + pos + 1 > buf.length) {
			throw new ParseException(
					String.format(
							"Insufficient data for bin LLVARBCD field %d, pos %d: need %d, only %d available",
		...
		if (custom == null) {
			...
			try {
				...
				throw new ParseException(
						String.format(
								"Insufficient data for LLVARBCD field %d, pos %d length %d",
								field, pos, l), pos);
			}
		} else {
