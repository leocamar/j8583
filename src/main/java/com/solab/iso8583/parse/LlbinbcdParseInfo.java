 ...
 */
public class LlbinbcdParseInfo extends FieldParseInfo {

	public LlbinbcdParseInfo() {
		super(IsoType.LLBINBCD, 0);
	}
	...
	public <T> IsoValue<?> parse(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid LLBINBCD field %d position %d", field, pos), pos);
		} else if (pos + 2 > buf.length) {
			throw new ParseException(String.format(
					"Insufficient LLBINBCD header field %d", field), pos);
		}
		length = decodeLength(buf, pos, 2);
		if (length < 0) {
			throw new ParseException(String.format(
					"Invalid LLBINBCD field %d length %d pos %d", field,
					length, pos), pos);
		}
		if (length + pos + 2 > buf.length) {
			throw new ParseException(
					String.format(
							"Insufficient data for LLBINBCD field %d, pos %d (LEN states '%s')",
	...
	public <T> IsoValue<?> parseBinary(final int field, final byte[] buf,
		...
		if (pos < 0) {
			throw new ParseException(String.format(
					"Invalid bin LLBINBCD field %d position %d", field, pos),
					pos);
		} else if (pos + 1 > buf.length) {
			throw new ParseException(String.format(
					"Insufficient bin LLBINBCD header field %d", field), pos);
		}
		final int l = Integer.parseInt(
				String.valueOf((((buf[pos] & 0xf0) >> 4) * 16) + (buf[pos] & 0x0f)));
		if (l < 0) {
			throw new ParseException(String.format(
					"Invalid bin LLBINBCD length %d pos %d", length, pos), pos);
		}
		if (l + pos + 1 > buf.length) {
			throw new ParseException(
					String.format(
							"Insufficient data for bin LLBINBCD field %d, pos %d: need %d, only %d available",
		...
		if (custom == null) {
			...
			try {
				...
				throw new ParseException(
						String.format(
								"Insufficient data for LLBINBCD field %d, pos %d length %d",
								field, pos, l), pos);
			}
		} else {
